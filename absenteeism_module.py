...
class absenteeism_model():
		...
		def load_and_clean_data(self, data_file):
			...
			df.rename(columns={'Pet':'Pets'},inplace = True)
			# we have included this line of code if you want to call the 'preprocessed data'
			self.preprocessed_data = df.copy()
			if 'Excessive Absenteeism' not in df.columns:
				df['Excessive Absenteeism'] = 0
			# we need this line so we can use it in the next functions
			self.data = self.scaler.transform(df)
			
			self.data = pd.DataFrame(self.data, columns=df.columns)



			trained_column_order = list(self.reg.feature_names_in_)

			self.data = self.data[trained_column_order]									
		...
		def predicted_outputs(self):
			if (self.data is not None):
				print("Model was trained on:\n", list(self.reg.feature_names_in_))
				print("Your data has columns:\n", list(self.data.columns))
